...
void restCallPostJsonAsync(string url, map <string, string> params) {
    CURL *curl;
    curl = curl_easy_init();
    if (curl) {
...
void restCallPostJson(string url, map <string, string> params) {
    thread t0(restCallPostJsonAsync, url, params);
    t0.detach();
}
